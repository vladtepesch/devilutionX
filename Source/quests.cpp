...
bool QuestLogIsOpen;               ///< \todo move to questbook
std::optional<CelSprite> pQLogCel; ///< \todo move to questbook
...
private:
	...
	int qlist[MAXQUESTS];  ///< indices of quests to display in quest log window. `firstFinishedEntry` are active quests the rest are completed
...
int QuestBook::cursorToSelection() const
{
...
void QuestBook::PrintQLString(const Surface &out, int x, int y, const char *str, bool marked, bool disabled) const
{
...
void QuestBook::draw(const Surface &out)
{
...
void QuestlogUp()
{
	questBook.selectionUp();
}

void QuestlogDown()
{
	questBook.selectionDown();
}
